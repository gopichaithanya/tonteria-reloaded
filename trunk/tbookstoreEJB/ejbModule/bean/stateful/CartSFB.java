...
//@RemoteHome(bean.stateful.interf.CartSFBHOME.class)
public class CartSFB implements CartSFBI, Serializable {
  ...
  //  @Override
	...
	//	System.out.println("Dimesione della lista di linee nell'ordine " + order.getLineItems().size());
		em.persist(order);
		// salvo i lineItem dell'ordine nel db
		for( Iterator<LineItem> i = order.getLineItems().iterator(); i.hasNext(); ) {
			LineItem tmpLI = i.next();
		//	System.out.println("lineItem for the book: "+tmpLI.getPk().getBook().getIsbn() +", quantity: " + tmpLI.getQuantity());
			em.persist(tmpLI);
		}
    }
